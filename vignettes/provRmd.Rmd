...
* At the end of the R Markdown script:
    ...
    * option 2 (fancy): use `prov_wrapup()` to include an R Markdown footer; this function includes a call to `script_prov()` and then renders provenance information into an optional summary (system and session info), workflow (diagram showing the basic steps of the analysis), and/or table (tabular file-by-file info on provenance)
...
# Accessing and displaying provenance

While the `script_prov()` function collects all the system and session info and attaches it to the provenance log, it only saves the log, and does nothing to display the results.  The information for the provenance log can be accessed from the named list returned from `script_prov()` or the `.script_track` dataframe; but there is a built-in way to access this info easily.

## Using `prov_wrapup()` to create default footers
The `prov_wrapup()` function includes a call to `script_prov()`, and then provides options to include a brief summary (including just system and session information), a workflow diagram (using `prov_plot()`, a flow chart showing the basic processing steps tracked throughout the script run, as well as the inputs and outputs), and/or a table containing specific information on each tracked file.  Creating a footer based on the example above (__NOTE: to display properly,__ the `prov_wrapup()` function must be called from within a code chunk with the `results = 'asis'` chunk option):
