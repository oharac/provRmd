...
* At the end of the R Markdown:
    * option 1 (basic): `script_prov()` to add a built-in R Markdown provenance footer to attach session and system info, and save a provenance log file.
    * option 2 (fancy): include a pre-built R Markdown footer to execute option 1, and then turn it into pretty graphs and tables within your rendered R Markdown document.
    * option 3 (hard core): design your own custom footer using `script_prov()` and `plot_prov()`
...
``` {r fig setup, echo = FALSE, warning = FALSE, message = FALSE}
...
As the script runs, file provenance info is stored in a hidden global object, `.prov_track`, which can be accessed like any R object.  Here we use the built-in `iris` dataset to create a new .csv file that will be used as an input for later chunks. 
...
### The `.prov_track` variable

`.prov_track` is the hidden global variable that stores provenance info for each tracked file, including:

`r paste(names(.prov_track), sep = ', ')`

Quick peek at the first five variables within `.prov_track` (note, sequence starts at 1 here, but at the _end_ of the knitting session, the master Rmd script will be assigned a sequence of 0):

`r knitr::kable(.prov_track[, 1:5])`
...
Note the new row at the end of `.prov_track()`:
...
### Using the output of `script_prov()`

The named list that is returned from `script_prov()` contains the following information:
`r paste(names(script_out_df), sep = ', ')`
...
Just as `.prov_track` can be accessed directly, we can also view the contents of `.script_track`.  The full list of variables:
`r paste(names(.script_track), sep = ', ')`

A glimpse of `.script_track` (a few select columns):
`r knitr::kable(.script_track[, 4:9] %>% arrange(sequence) %>% head())`
...
``` {r plot_prov, echo = TRUE, message = FALSE, warning = FALSE}
...
``` {r broken_link_example1, echo = TRUE, message = FALSE, warning = FALSE}
...
```{r broken_link_example2, echo = TRUE, message = FALSE, warning = FALSE}
z <- y %>%
  mutate(new_col2 = 'more words')
write_csv(z, 'table_z.csv')
```

``` {r prov_plot 2, echo = FALSE, message = FALSE, warning = FALSE}
script_prov()
DiagrammeR::render_graph(plot_prov(.script_track, plot_dir = 'TB'))
```
...
``` {r reg_link_ex1, echo = TRUE, message = FALSE, warning = FALSE}
prov_setup() ### to clear prior provenance tracking
x <- read_csv('iris_table1.csv')
y <- x %>%
  mutate(new_col = 'words')
...
```{r reg_link_ex2, echo = TRUE, message = FALSE, warning = FALSE}
...
Now we can clearly see that the chunk that output `table_z.csv` used `table_y.csv` as an input.
...
``` {r if_link1, echo = TRUE, message = FALSE, warning = FALSE}
prov_setup() ### to clear prior provenance tracking
...
```{r if_link2, echo = TRUE, message = FALSE, warning = FALSE}
y <- read_csv('table_y.csv')
z <- y %>%
  mutate(new_col2 = 'more words')
write_csv(z, 'table_z.csv')
...
The `provRmd` package includes several pre-built footers that call `script_prov()` and output provenance information in a few different ways (table, flow chart, text).  These footers are in a 'footer' folder within the `provRmd` package, and can be accessed directly calling `system.file()`.  For example:

``` {r, echo = TRUE}
list.files(system.file('footer', package = 'provRmd'))
```

Currently, the `prov_setup()` function copies these footers to a directory named 'prov' within the R Markdown working directory.  This allows them to easily be called at the end of the R Markdown, via a knitr code chunk with the 'child' option: `{r, child = 'prov/prov_ftr.Rmd'}`

## Custom footers

Footers can be customized easily to show different information as desired, as a separate .Rmd or simply within the main Rmd itself.  The basic idea is to call `script_prov()`, and then create desired outputs as a combination of the list object returned from `script_prov()`, the `.script_track` dataframe object, and the `plot_prov()` function (to assemble a workflow diagram).
