...
script_prov()
DiagrammeR::render_graph(plot_prov(.script_track, plot_dir = 'LR'))
```

From this diagram, there's no easy way to see the relationship between dataframe `y` and dataframe `z`.
...
Chunk 2 (called `reg_link_example2`); `y` is used as an input to create `z`, but this time object `y` is reloaded from disk rather than used from memory.  This seems redundant but can be an advantage, to make sure `y` is always the correct object:
...
Now we can clearly see that the chunk that output dataframe `z` used dataframe `y` as an input.
